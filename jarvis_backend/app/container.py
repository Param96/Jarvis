...
from jarvis_backend.audio.stt import SpeechToText, FasterWhisperSTT, SpeechRecognitionSTT
...
def build_container(settings: Settings) -> Container:
    ...
    if settings.cloud_model_provider != "disabled" and settings.cloud_api_key:
        ...
        if settings.cloud_coding_model_name:
            cloud_coding_model = OpenAICompatibleChatModel(base_url, settings.cloud_api_key, settings.cloud_coding_model_name)
    ...
    triage_agent = Agent(
        ...
        model=triage_model,
        allowed_tools=["current_time", "read_text_file", "web_search", "transfer_agent"],
        skills=["hello_world"]
    )
    ...
    coder_agent = Agent(
        ...
        model=coder_model,
        allowed_tools=["write_text_file", "terminal_command", "read_text_file", "index_codebase", "search_codebase", "run_background_command"],
