...
class WriteTextFileTool(Tool):
    ...
    async def run(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        return ToolResult(name=self.spec.name, ok=True, output=f"Successfully wrote to {path}")
...
class WebSearchTool(Tool):
    ...
    async def run(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(name=self.spec.name, ok=True, output=output or "No results found.")
        except Exception as e:
            return ToolResult(name=self.spec.name, ok=False, output=f"Search failed: {e}")
...
class TransferAgentTool(Tool):
    ...
    spec = ToolSpec(
        ...
        parameters={"agent_name": {"type": "string", "description": "The name of the agent to transfer to (e.g. 'CoderAgent')"}},
    )
...
class IndexCodebaseTool(Tool):
    ...
    spec = ToolSpec(
        ...
        parameters={"path": {"type": "string", "description": "Absolute path to the file to index."}},
    )
    ...
    async def run(self, arguments: dict[str, Any]) -> ToolResult:
        if not isinstance(self._memory, PgVectorMemoryStore):
            return ToolResult(name=self.spec.name, ok=False, output="Codebase indexing requires PgVectorMemoryStore.")
        ...
        try:
            ...
            return ToolResult(name=self.spec.name, ok=True, output=f"Successfully indexed {path}")
        except Exception as e:
            return ToolResult(name=self.spec.name, ok=False, output=f"Failed to index {path}: {e}")
...
class SearchCodebaseTool(Tool):
    ...
    async def run(self, arguments: dict[str, Any]) -> ToolResult:
        if not isinstance(self._memory, PgVectorMemoryStore):
            return ToolResult(name=self.spec.name, ok=False, output="Codebase search requires PgVectorMemoryStore.")
            
        query = str(arguments["query"])
        try:
            results = await self._memory.search_code(query)
            if not results:
                return ToolResult(name=self.spec.name, ok=True, output="No relevant code found.")
...
class FetchUrlTool(Tool):
    ...
    async def run(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(name=self.spec.name, ok=False, output=f"Failed to fetch {url}: {e}")
...
class RunBackgroundCommandTool(Tool):
    ...
    def __init__(self, settings: Settings, permissions: PermissionService, task_queue: BackgroundTaskQueue) -> None:
        self._settings = settings
        self._permissions = permissions
    ...
    async def run(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        return ToolResult(name=self.spec.name, ok=True, output=f"Started in background. Task ID: {task_id}")
