...
class AIOrchestrator:
    def __init__(self):
        self.local_llm_url = os.getenv("LOCAL_LLM_URL", "http://host.docker.internal:11434/api/generate")
    ...
    async def route_task(self, device_id: str, intent: str, context: dict = None, model_override: str = None):
        ...
        if target_model == "GPT-4o" or target_model.startswith("OpenRouter"):
            ...
            try:
                res = requests.post(self.local_llm_url, json={
                    "model": target_model,
        ...
        return {"status": "routed", "destination": target_model, "response": response_text}
